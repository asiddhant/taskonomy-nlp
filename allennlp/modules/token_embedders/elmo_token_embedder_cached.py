...
def generate_keys(words_list,input_ids):
    keys = []
    lens = []
    for inp,words in zip(input_ids,words_list):
        inp = inp.detach().cpu().numpy().tolist()
        lens.append(len([k for k in inp if set(k)!=set([0])]))
        key = tuple(words)
        keys.append(key)
    return keys,lens

@TokenEmbedder.register("elmo_token_embedder_cached")
class ElmoTokenEmbedderCached(TokenEmbedder):
    ...
    @classmethod
    def from_params(cls, vocab: Vocabulary, params: Params) -> 'ElmoTokenEmbedderCached':  # type: ignore
    ...
    def retrieve_from_cache(self,keys):
        embeddings = []
        for key in keys:
            embedding = self.cache[key].to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
